...
for v in df.iloc[3]:  # for alle verdier i 3. rad. df.iloc[3][2] gir 3 rads andre verdi
    print(v, end=" ")  # -> 3.0 -0.025537001 0.201338485 2.0927856 1.0	(ferdig konvertert til float)
 """
#bin\OpenPoseDemo.exe --video examples\test.mp4 --face --hand --write_json output_json_folder/
...
def tocsv():
    outfile_path = 'backend/preprocessing/dummy_data/skeleton/asd.csv'

    protoFile = "backend/preprocessing/dummy_data/skeleton/pose_deploy_linevec_faster_4_stages.prototxt"
    weightsFile = "backend/preprocessing/dummy_data/skeleton/pose_iter_160000.caffemodel"
    net = cv2.dnn.readNetFromCaffe(protoFile, weightsFile)
                                                                   #368 386
    data, input_width, input_height, threshold, frame_number = [], 368, 386, 0.1, 0

    input_source = "backend/preprocessing/dummy_data/skeleton/newest.mp4"
