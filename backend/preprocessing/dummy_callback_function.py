...
def eyetracker_callback(q):
    ...
    {'index': 386.0, 'initTime': 111.0, 'endTime': 203.0, 'lpup': 3.664, 'rpup': 3.572, 'fx': 1054.0, 'fy': 720.0}
    ...
    def fileline_to_dictionary(line):
        ...
        dictionary_strings = ["index", "initTime", "endTime", "lpup", "rpup", "fx", "fy"]
        eyetracker_data = {}
        line = line[:-1].split(",")
    ...
    eyetracker = open(os.path.join(os.path.dirname(__file__), 'dummy_data', 'eye-tracking', 'ET-data-S001.csv'), "r")
    eyetracker.readline()  # pass meta info: index,initTime,endTime,lpup,rpup,fx,fy
    while True:
        last_point = eyetracker.readline()
        if last_point == '':  # end of file
            print("end of file")
            break
        q.put(fileline_to_dictionary(last_point))  # push a dict to the multistream queue
