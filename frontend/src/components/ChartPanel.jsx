...
export default function ChartPanel({ name, graphData, dataStats }) {
  ...
  function formatJointStats(jointStats) {
    const result = Object.keys(jointStats).map((key) => {
      const res = {};
      res.value = jointStats[key];
      res.name = key;
      return res;
    });
    return result;
  ...
  return (
    <div className="Extended-graph-container Box">
      ...
      { graph === 'line'
        ...
        : <BarGraph stat="max" dataStats={formatStats(dataStats)} />}
    </div>
  );
}
